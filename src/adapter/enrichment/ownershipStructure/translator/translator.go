...
func translateShareholders(shareholdersResponse contracts.Shareholders) ([]entity.Shareholder, error) {

	shareholders := make([]entity.Shareholder, 0)

	for _, shareholderResponse := range shareholdersResponse {

		shareholder, err := translateShareholder(shareholderResponse)
		if err != nil {
			return nil, err
		}
		...
		domainShareholders, err := translateShareholders(shareholderResponse.Shareholders)
		if err != nil {
			return nil, err
		}
...
func translateShareholder(response contracts.Shareholder) (*entity.Shareholder, error) {
	...
	shareholderID := uuid.NewSHA1(core.GetUuidNamespace(), []byte(response.DocumentNumber))
	...
	shareholder := &entity.Shareholder{
		...
		Person: entity.Person{
			...
			Name:           response.Name,
			PersonType:     translatePersonType(response.Type),
...
func translatePersonType(personType string) string {
	switch personType {
	case "LEGAL_ENTITY":
		return "COMPANY"
	default:
