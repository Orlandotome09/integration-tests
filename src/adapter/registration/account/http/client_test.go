package accountClient

import (
	"bitbucket.org/bexstech/temis-compliance/src/adapter/registration/account/http/contracts"
	"bytes"
	"encoding/json"
	"github.com/google/uuid"
	"net/http"
	"net/http/httptest"
	"reflect"
	"testing"
)

func TestGet(t *testing.T) {
	response := &contracts.AccountResponse{AccountID: uuid.MustParse("4d2d0cc3-f173-4a78-86c8-5b9de2fbf8d2")}
	responseBody, _ := json.Marshal(response)

	server := mockServer("GET", 200, nil, responseBody)
	defer server.Close()

	client := New(server.Client(), server.URL)

	expected := response
	received, err := client.Get("111")

	if !reflect.DeepEqual(expected, received) {
		t.Errorf("\nExpected: %v \nGot: %v\n", expected, received)
	}

	if err != nil {
		t.Errorf("\nExpected err nil")
	}
}

func TestGet_NoContent(t *testing.T) {
	response := &contracts.AccountResponse{AccountID: uuid.MustParse("4d2d0cc3-f173-4a78-86c8-5b9de2fbf8d2")}
	responseBody, _ := json.Marshal(response)

	server := mockServer("GET", 204, nil, responseBody)
	defer server.Close()

	client := New(server.Client(), server.URL)

	var expected *contracts.AccountResponse = nil
	received, err := client.Get("111")

	if !reflect.DeepEqual(expected, received) {
		t.Errorf("\nExpected: %v \nGot: %v\n", expected, received)
	}

	if err != nil {
		t.Errorf("\nExpected err nil")
	}
}

func TestGet_ServerError(t *testing.T) {
	response := &contracts.AccountResponse{AccountID: uuid.MustParse("4d2d0cc3-f173-4a78-86c8-5b9de2fbf8d2")}
	responseBody, _ := json.Marshal(response)

	server := mockServer("GET", 500, nil, responseBody)
	defer server.Close()

	client := New(server.Client(), server.URL)

	var expected *contracts.AccountResponse = nil
	received, err := client.Get("111")

	if !reflect.DeepEqual(expected, received) {
		t.Errorf("\nExpected: %v \nGot: %v\n", expected, received)
	}

	if err == nil {
		t.Errorf("\nExpected err not nil")
	}
}

func TestFind(t *testing.T) {
	response := []contracts.AccountResponse{{AccountID: uuid.New()}, {AccountID: uuid.New()}}
	responseBody, _ := json.Marshal(response)

	server := mockServer("GET", 200, nil, responseBody)
	defer server.Close()

	client := New(server.Client(), server.URL)

	expected := response
	received, err := client.Find("111")

	if !reflect.DeepEqual(expected, received) {
		t.Errorf("\nExpected: %v \nGot: %v\n", expected, received)
	}

	if err != nil {
		t.Errorf("\nExpected err nil")
	}
}

func TestFind_err(t *testing.T) {
	response := []contracts.AccountResponse{{AccountID: uuid.New()}, {AccountID: uuid.New()}}
	responseBody, _ := json.Marshal(response)

	server := mockServer("GET", 500, nil, responseBody)
	defer server.Close()

	client := New(server.Client(), server.URL)

	var expected []contracts.AccountResponse = nil
	received, err := client.Find("111")

	if !reflect.DeepEqual(expected, received) {
		t.Errorf("\nExpected: %v \nGot: %v\n", expected, received)
	}

	if err == nil {
		t.Errorf("\nExpected err not nil")
	}
}

func TestCreate(t *testing.T) {
	request := &contracts.CreateAccountRequest{ProfileID: "1111", BankCode: "1111"}
	requestBody, _ := json.Marshal(request)

	response := &contracts.AccountResponse{AccountID: uuid.MustParse("4d2d0cc3-f173-4a78-86c8-5b9de2fbf8d2")}
	responseBody, _ := json.Marshal(response)

	server := mockServer("POST", 201, requestBody, responseBody)
	defer server.Close()

	client := New(server.Client(), server.URL)

	var expected = response
	received, err := client.CreateInternal(request)

	if !reflect.DeepEqual(expected, received) {
		t.Errorf("\nExpected: %v \nGot: %v\n", expected, received)
	}

	if err != nil {
		t.Errorf("\nExpected err nil")
	}
}

func TestCreate_ServerError(t *testing.T) {
	request := &contracts.CreateAccountRequest{ProfileID: "1111", BankCode: "1111"}
	requestBody, _ := json.Marshal(request)

	response := &contracts.AccountResponse{AccountID: uuid.MustParse("4d2d0cc3-f173-4a78-86c8-5b9de2fbf8d2")}
	responseBody, _ := json.Marshal(response)

	server := mockServer("POST", 500, requestBody, responseBody)
	defer server.Close()

	client := New(server.Client(), server.URL)

	var expected *contracts.AccountResponse = nil
	received, err := client.CreateInternal(request)

	if !reflect.DeepEqual(expected, received) {
		t.Errorf("\nExpected: %v \nGot: %v\n", expected, received)
	}

	if err == nil {
		t.Errorf("\nExpected err not nil")
	}
}

//-------------------------------MockServer--------------------------------------
func mockServer(method string, responseStatus int, requestBody []byte, responseBody []byte) *httptest.Server {

	req := httptest.NewRequest(method, "http://test", bytes.NewBuffer(requestBody))
	responseRecorder := httptest.NewRecorder()
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(responseStatus)
		w.Write(responseBody)
	})
	handler.ServeHTTP(responseRecorder, req)
	server := httptest.NewServer(handler)

	return server
}
