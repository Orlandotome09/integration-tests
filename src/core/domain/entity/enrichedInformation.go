...
func (enrichedInformation EnrichedInformation) FilterCafProvider() *Provider {
	var cafProvider Provider
	for _, provider := range enrichedInformation.Providers {
		if provider.ProviderName == ProviderCAF {
			cafProvider = provider
			return &cafProvider
		}
	}
